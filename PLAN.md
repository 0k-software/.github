...
Four skill files (`create-issue`, `create-pr`, `plan-init`, `fix`) currently
use the `gh` CLI for all GitHub interactions. `gh` is unavailable in remote/web
Claude Code sessions. This plan replaces every `gh` call with a direct `curl`
call to the GitHub REST or GraphQL API, making skills work in any environment.
...
**File:** `0k/skills/fix/SKILL.md`

This is the most `gh`-heavy skill. Replace each call in order:
...
**A1 — Fetch reactions on a comment (replaces `--paginate`):**
...
curl -s -X POST \
  ...
  -d '{"content": "white_check_mark"}' | jq '.id'
```
...
**B1 — Fetch reactions on an issue comment:**
