...
- The **Proposal** section (present in all templates) must contain the full
  spec detail from the brainstorming session. Structure it freely — use
  headings, paragraphs, or lists as the content warrants. Cover architecture,
  components, data flow, error handling, and testing where relevant, but do not
  impose headings that don't apply to the scope of the issue.
